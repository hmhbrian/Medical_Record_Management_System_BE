...
public class FcmTokenService {
    ...
    public ApiResponse<?> saveToken(FcmTokenRequest request) {
        User user = userRepo.findById(request.getUserId())
                .orElseThrow(() -> new InvalidInputException("User không tồn tại với id: " + request.getUserId()));
        ...
        if (existingTokenOpt.isPresent()) {
            ...
            // => Không lưu, chỉ cần cập nhật lại updated_at hoặc đảm bảo active = true nếu
            // cần
            FcmToken existingToken = existingTokenOpt.get();

            // đánh dấu là ACTIVE và cập nhật thời gian:
            if (!existingToken.isActive() || existingToken.getDeviceType() == null
                    || !existingToken.getDeviceType().equals(request.getDeviceType())) {
